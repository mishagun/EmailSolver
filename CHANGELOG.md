...
- [--:--] Rewrite README.md for TidyInbox rebrand — updated project name, added web frontend + TUI sections, keyboard shortcuts, AI insights, batch API, sender grouping features, updated project structure, self-hosting guide.
- [--:--] Add `paths-ignore` to deploy workflow — skip CI on README.md, docs/**, CHANGELOG.md, LICENSE changes.
- [--:--] Rename "emailsolver" to "tidyinbox" across all UI — web (Layout header, HTML title, API client class, localStorage token key), TUI (app class/title, client class, config env prefix to `TIDYINBOX_TUI_`, token path to `~/.tidyinbox/token`), and all related tests.
- [--:--] Fix email stats to show real Gmail inbox counts — was using `list_messages(max_results=1)` so counts were always 0 or 1. Now uses Gmail `labels.get(INBOX)` API which returns actual `messagesUnread` and `messagesTotal`. Added `get_inbox_counts` to `BaseEmailService` protocol + `GmailService`. Files: `protocols.py`, `gmail_service.py`, `emails.py` route, `fakes.py`, `test_email_routes.py`.
- [--:--] Fix AI insights: remove conflicting `animate-in stagger-3` class that caused insights to flash and disappear (CSS `opacity: 0` base from `animate-in` overrode the `insight-fade-in` animation). Also add insights generation to inbox scan (`_run_inbox_scan`) so both analysis types get insights. Files: `AnalysisPage.tsx`, `analysis_service.py`.
- [--:--] Add AI insights feature — witty one-liner observations generated by Claude after analysis verification. Stored as JSON array on `Analysis.ai_insights`. New `generate_insights` method on `BaseClassificationService` protocol + `ClaudeClassificationService` implementation. `analysis_service.py` calls it after verification (non-blocking — failures logged, don't break analysis). Frontend shows one insight at a time with fade animation, cycling on tab/category/sender navigation. Files: `db.py`, `schemas.py`, `protocols.py`, `classification_service.py`, `analysis_repository.py`, `analysis_service.py`, `analysis.py` routes, `types.ts`, `AnalysisPage.tsx`, `global.css`, `tui/models.py`, `fakes.py`, `fixtures.ts`. Alembic migration `f80622ade508`.

## 2026-03-10

- [23:30] Remove category merging from verification step — AI no longer condenses categories. Users keep all AI-generated categories as-is. Removed merge logic from `analysis_service.py`, removed `protected_categories` from `verify_categories` protocol/implementation, simplified verification prompt in `classification_service.py`. Removed 4 merge tests, updated classification test.
- [23:27] Auto-apply falls back to `mark_read` for non-primary categories when AI only recommends "keep". Updated `_auto_apply_actions` in `analysis_service.py`. Updated verification prompt to bias toward `mark_read` as first recommended action for inbox zero.
- [23:15] Replace Gmail `q=is:unread` search query with `labelIds=["UNREAD"]` for reliable unread email filtering. Gmail search index is eventually consistent and can miss/include wrong emails. `labelIds` does direct label lookup. Replaced `query: str` with `unread_only: bool` across all layers: schemas, DB model, protocols, gmail_service, analysis_service, routes, TUI, web frontend. Added alembic migration. Split `test_analysis_service.py` (1354 lines) into 4 focused files + shared helpers module. Files: `gmail_service.py`, `protocols.py`, `schemas.py`, `db.py`, `analysis_service.py`, `analysis.py`, `emails.py`, `tui/models.py`, `tui/client.py`, `tui/screens/dashboard.py`, `tui/screens/analysis.py`, `web/src/api/types.ts`, `web/src/pages/DashboardPage.tsx`, `web/src/pages/AnalysisPage.tsx`, `web/src/test/fixtures.ts`, tests.
- [--:--] Batch API for large analyses + default 500 emails — when >500 emails, AI classification uses Anthropic Message Batches API (50% cheaper, async). Added `submit_batch_classification`, `check_batch_status`, `retrieve_batch_results` to classification service + protocol. Analysis service branches to batch/realtime based on threshold. Frontend warns about batch processing time. Default max_emails changed 100→500 across backend, web, and TUI. Files: `classification_service.py`, `analysis_service.py`, `protocols.py`, `schemas.py`, `analysis.py` routes, `DashboardPage.tsx`, `AnalysisPage.tsx`, `types.ts`, `tui/models.py`, fakes, tests.
- [--:--] Fix AI analysis defaulting to Google categories — verification step was merging base categories (newsletters→promotions, receipts→updates). Added protected category guard in `analysis_service.py`, updated verification prompt in `classification_service.py` to preserve base + user-specified custom categories, updated protocol in `protocols.py`. Added 4 tests.
