...
class TestRunInboxScan:
    ...
    async def test_happy_path(self) -> None:
        ...
        classified_email_repo = MagicMock()
        classified_email_repo.bulk_create = AsyncMock(
            return_value=[_make_classified_email(category="promotions")]
        )
        classified_email_repo.bulk_update_action_taken = AsyncMock()
        classified_email_repo.bulk_record_action = AsyncMock()
        classified_email_repo.pop_last_action = AsyncMock(return_value={})
    ...
    async def test_no_classification_service_calls(self) -> None:
        ...
        classified_email_repo = MagicMock()
        classified_email_repo.bulk_create = AsyncMock(
            return_value=[_make_classified_email(category="updates")]
        )
        classified_email_repo.bulk_update_action_taken = AsyncMock()
        classified_email_repo.bulk_record_action = AsyncMock()
        classified_email_repo.pop_last_action = AsyncMock(return_value={})
    ...
    async def test_auto_apply_uses_default_actions(self) -> None:
        # Arrange
        spam_meta = _make_email_metadata(msg_id="msg-spam")
        spam_meta.gmail_category = "spam"
        promo_meta = _make_email_metadata(msg_id="msg-promo")
    ...
    async def test_batched_fetching(self) -> None:
        ...
        emails_batch1 = [
            _make_email_metadata(msg_id=f"msg-{i}") for i in range(50)
        ]
        for e in emails_batch1:
            e.gmail_category = "primary"
        emails_batch2 = [
            _make_email_metadata(msg_id=f"msg-{i}") for i in range(50, 75)
        ]
        ...
        created_batch1 = [
            _make_classified_email(email_id=i + 1, msg_id=f"msg-{i}", category="primary")
            for i in range(50)
        ]
        created_batch2 = [
            _make_classified_email(email_id=i + 51, msg_id=f"msg-{i}", category="primary")
